...
const SEED_PRODUCTS = DATA.products.map(p => ({ ...p, category:'Metales', sku:`PROD-${String(p.id).padStart(3,'0')}`, description:'', active:true }));
...
function GlobalSearch({ clients, quotes, products, onNavigate, onClose }) {
  ...
  const results = useMemo(() => {
    ...
    return [
      ...
      ...products.filter(p=>p.name.toLowerCase().includes(lq)).map(p=>({type:'Producto',label:p.name,sub:`$${p.price}`,tab:'products'})),
    ].slice(0,8);
    ...
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-start justify-center pt-24 px-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden border border-slate-200">
        <div className="flex items-center gap-3 p-4 border-b border-slate-100">
          ...
          <input autoFocus type="text" placeholder="Buscar clientes, cotizaciones, productos..." value={q} onChange={e=>setQ(e.target.value)}
...
export default function App() {
  ...
  const [products,        setProducts]        = useLocalStorage('cc_products',     SEED_PRODUCTS);
  ...
  const handleLogin = user => {
    setCurrentUser(user);
    audit(`Inicio de sesión — ${user.role}`, user.id, 'Sistema');
  };

  const handleLogout = () => {
    audit(`Cierre de sesión — ${currentUser?.role}`, currentUser?.id, 'Sistema');
    setCurrentUser(null);
    setActiveTab('dashboard');
  };

  const resetDemo = () => {
    if (!window.confirm('¿Resetear todos los datos al estado inicial de la demo?')) return;
    localStorage.clear();
    ...
    return {
      ...
      productLines:      products.map(p => ({
        ...
        amount: approved.reduce((s,q) => {
          return s + (q.items||[]).filter(it => it.productId === p.id).reduce((ss,it) => ss + it.quantity*it.price, 0);
        }, 0),
      })),
    };
  }, [campaigns, clients.length, quotes, products]);

  // ── CRUD handlers with audit ──
  ...
  const saveAudit    = a => { setAudits(cur=>cur.map(x=>x.id===a.id?a:x)); audit(`Auditoría de calidad guardada — score ${a.score}`, a.id, 'Seguridad'); notify('success',`Auditoría ${a.id} guardada.`); };
  const submitAnswer = (mid, ans) => { setTrainingModules(cur=>cur.map(m=>{ if(m.id!==mid) return m; const ok=ans===m.correctAnswer; audit(ok?'Módulo capacitación aprobado':'Intento fallido en capacitación', mid, 'Capacitacion'); return {...m,selectedAnswer:ans,status:ok?'Completado':'En curso',feedback:ok?'Respuesta correcta. Módulo aprobado.':'Respuesta incorrecta. Revisa el contenido.'}; })); notify('info','Evaluación procesada.'); };
  const runAudit     = () => { const log={id:`LOG-${String(securityLogs.length+1).padStart(3,'0')}`,module:'Seguridad',event:'Auditoría de Seguridad Ejecutada',user:currentUser?.name||'Admin',role:currentUser?.role||'Administrador',date:ts(),obj:encryptionActive?'AES-256':'REVISION_MANUAL',alert:!encryptionActive}; setSecurityLogs(cur=>[log,...cur]); notify('success','Auditoría ejecutada.'); };
  ...
  const toggleChannel    = chId      => { setChannels(cur=>cur.map(ch=>{ if(ch.id!==chId) return ch; const en=!ch.enabled; return {...ch,enabled:en,status:en?'Conectado':'Desconectado',latency:en?(ch.latency==='--'?'22ms':ch.latency):'--'}; })); audit('Estado canal modificado', chId, 'Sistema'); notify('info','Canal actualizado.'); };
  ...
  const saveCall   = data    => {
    ...
    if (data.scheduleFollowUp?.date) {
      setFollowUps(cur=>[...cur,{ id:`FU-${Date.now()}`, clientName:data.companyName, agent:currentUser?.name||'Agente', date:data.scheduleFollowUp.date, time:'', priority:'Media', channel:'Llamada', notes:data.notes||'Follow-up automático.', status:'Pendiente', createdAt:ts().slice(0,10) }]);
      audit('Follow-up automático generado desde llamada', data.companyName, 'Follow-up');
    }
  };
    ...
    switch (activeTab) {
      ...
      case 'calls':         return (
        <div className="space-y-5">
          <div><h2 className="text-2xl font-bold text-slate-900">Consola de Llamadas</h2><p className="text-slate-500 text-sm">Inicia llamadas, tipifica y genera follow-ups automáticos.</p></div>
          <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm">
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Historial reciente</p>
            <div className="space-y-1.5">
              {callHistory.slice(0,5).map((c,i) => (
                <div key={i} className="flex items-center gap-3 text-xs p-2 rounded-xl hover:bg-slate-50">
                  <span className="font-mono text-slate-400 flex-shrink-0">{c.timestamp?.slice(0,10)}</span>
                  <span className="font-bold text-slate-700 flex-1 truncate">{c.companyName}</span>
                  <span className={`px-2 py-0.5 rounded font-bold text-[10px] flex-shrink-0 ${c.disposition==='CONTACTO_EFECTIVO'?'bg-emerald-100 text-emerald-700':c.disposition==='SOLICITA_COTIZACION'?'bg-blue-100 text-blue-700':'bg-slate-100 text-slate-600'}`}>{c.disposition?.replace(/_/g,' ')}</span>
                </div>
              ))}
              {callHistory.length===0 && <p className="text-slate-400 text-center py-4 text-xs">Sin llamadas registradas.</p>}
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {clients.map(client => {
              ...
              return (
                <div key={client.id} className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm hover:shadow-md hover:border-blue-200 transition-all group">
                  <div className="flex items-start justify-between mb-3">
                    <div><h3 className="font-bold text-slate-800">{client.name}</h3><p className="text-xs text-slate-400">{client.contact}{client.position?` · ${client.position}`:''}</p></div>
                    <span className="text-[10px] font-bold uppercase bg-slate-50 text-slate-500 px-2 py-0.5 rounded border border-slate-200">{client.sector}</span>
                  </div>
                  <p className="text-xs text-slate-500 mb-1 flex items-center gap-1"><Phone size={10}/> {client.phone}</p>
                  {prev.length>0 && <p className="text-[10px] text-slate-400 mb-3">{prev.length} llamada(s) prev. — {prev[0]?.disposition?.replace(/_/g,' ')}</p>}
                  <button type="button" onClick={()=>startCall({contactName:client.contact,companyName:client.name,phoneNumber:client.phone})}
                    className="w-full flex items-center justify-center gap-2 py-2.5 bg-emerald-600 text-white rounded-xl text-xs font-bold hover:bg-emerald-700 transition-all">
      ...
      case 'opportunities': return <OpportunityPipeline clients={clients} products={products} onNotify={notify}/>;
      ...
      case 'security':      return <DatabaseSecurity encryptionActive={encryptionActive} logs={securityLogs} onToggleEncryption={()=>{ setEncryptionActive(v=>!v); audit('Estado cifrado modificado','AES-256','Seguridad'); notify('info','Cifrado actualizado.'); }} onRunAudit={runAudit}/>;
      default:              return null;
    }
  };
  ...
  return (
    <div className="flex min-h-screen bg-slate-100 font-sans text-slate-900">
                    ...
                    return (
                      <li key={item.id}>
                        <button type="button" onClick={()=>setActiveTab(item.id)}
                          className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs font-semibold transition-all relative ${active?'bg-blue-600 text-white shadow-lg shadow-blue-900/40':'text-slate-400 hover:bg-slate-800/70 hover:text-slate-200'}`}>
                          <Icon size={15}/>
        ...
        <header className="sticky top-0 z-30 bg-white/80 backdrop-blur border-b border-slate-200 px-8 py-3 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">
              Sesión: <span className="text-slate-700">28 Mar 2026</span>
            </div>
            {overdueFollowUps > 0 && (
          ...
          <div className="flex items-center gap-3">
            ...
            <button type="button" onClick={()=>setShowSearch(true)}
              ...
              <Search size={13}/> Buscar <kbd className="text-[9px] font-black text-slate-400 bg-white px-1.5 py-0.5 rounded border border-slate-200 ml-1">⌘K</kbd>
            </button>
            <div className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider border ${currentUser.role==='Supervisor'?'bg-emerald-50 text-emerald-700 border-emerald-200':currentUser.role==='Administrador'?'bg-violet-50 text-violet-700 border-violet-200':'bg-blue-50 text-blue-700 border-blue-200'}`}>
              {currentUser.role}
            </div>
          </div>
        </header>
      ...
      {showSearch && <GlobalSearch clients={clients} quotes={quotes} products={products} onNavigate={tab=>{setActiveTab(tab);}} onClose={()=>setShowSearch(false)}/>}
      {activeCall && <CallConsole activeCall={activeCall} callHistory={callHistory} onHangUp={hangUp} onSave={saveCall}/>}
    </div>
  );
}
